...
public class WrapMesh : MonoBehaviour
{
    ...
    public IChildMeshFilter childMeshFilter;
    public void SetUp(IChildMeshFilter meshFilter)
    {
        childMeshFilter = meshFilter;
        _meshFilters = meshFilter.GetMeshFilters();
        _materials = meshFilter.GetMaterials();
    ...
    void SetupWrapMeshMaterials()
    {
        ...
        meshRenderer.material = random_mat;
    }
    ...
    public void SetupWrapMesh()
    {
        ...
        if (peelingMode)
        {
            var wrap_material = meshRenderer.material;
            for (var mi = 0; mi < _materials.Count; mi++)
            {
                var mat = Instantiate(wrap_material);
                mat.mainTexture = _materials[mi].mainTexture;
                mat.color = _materials[mi].color;
                _materials[mi] = mat;
            }
    ...
    public WrapMesh MakeInstanceFromModel(StageTarget target, Transform parent = null)
    {
        ...
        //duplicate target model inside new wrap mesh
        StageTarget newTarget = Instantiate(target);
        newTarget.gameObject.SetActive(true);
        newTarget.transform.SetParent(result.transform);

        result.originalTransform = newTarget.transform;
        result.transform.position = newTarget.transform.position;
        result.SetUp(newTarget);
